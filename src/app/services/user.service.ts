...
export class UserService {
  ...
  getAll(page: number = 1): Promise<IResponse> {
    return lastValueFrom(
      this.httpClient.get<IResponse>(`${this.apiUrl}?page=${page}`)
    );
  }
  ...
  update(id: string, user: IUsuario): Promise<IUsuario> {
    return lastValueFrom(
      this.httpClient.put<IUsuario>(this.apiAction + id, user)
    );
  }

  insert(id: string, user: IUsuario): Promise<IUsuario> {
    return lastValueFrom(
      this.httpClient.post<IUsuario>(this.apiAction + id, user)
    );
  }
